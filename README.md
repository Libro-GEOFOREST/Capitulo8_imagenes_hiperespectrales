...
$$ rho_{p} = pi · L_{lambda} · d^2 · ESUN_{lambda] · cos theta_{S} $$

$$ &rho_{p} = &pi · L_{&lambda} · d^2 · ESUN_{&lambda] · cos &theta_{S} $$

Donde $rho_p$ es la reflectancia exoatmosférica en el satélite, $L_{lambda}$ es la radiancia en W $m^(-2) s r^(-1) mu m^(-1)$, d es la distancia entre la tierra y el sol en unidades astronómicas el día de la adquisición de la imagen, $ESUN_{lambda}$ es la irradiancia solar exoatmosférica media y $theta_{S}$ es el ángulo zenital solar en grados.
